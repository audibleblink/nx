...
func main() {
	...
	// Default behavior: if no subcommand specified, treat remaining args as server command
	// This maintains backward compatibility
	var serverCmd config.ServerCommand
	serverParser := flags.NewParser(&serverCmd, flags.Default)
	if _, err := serverParser.Parse(); err != nil {
		if flagsErr, ok := err.(*flags.Error); ok && flagsErr.Type == flags.ErrHelp {
			os.Exit(0)
		}
		logerr.Fatal("Failed to parse arguments:", err)
	...
	executeServerCommand(&serverCmd)
}

func executeServerCommand(cfg *config.ServerCommand) {
...
func executeExecCommand(cfg *config.ExecCommand) {
	...
	if err := executePluginOnPane(pluginManager, cfg.Args.Script, target); err != nil {
...
// executePluginOnPane executes a plugin script on a specific pane
func executePluginOnPane(pluginManager *plugins.Manager, pluginName string, target *tmux.PaneTarget) error {
	pluginPath := filepath.Join(pluginManager.GetPluginDir(), pluginName+".sh")
	...
	for scanner.Scan() {
		...
		// Get the tmux manager from the plugin manager to execute on the specific pane
		tmuxManager, err := tmux.NewManager(target.Session)
		if err != nil {
			return fmt.Errorf("failed to get tmux manager: %w", err)
		}

		err = tmuxManager.ExecuteOnPane(target, line)
		if err != nil {
			logerr.Warn("plugin command failed:", err)
		}

		// Default sleep between commands (500ms)
